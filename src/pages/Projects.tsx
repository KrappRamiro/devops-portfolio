...
export const Projects = () => {
  const [activeFilter, setActiveFilter] = useState<keyof typeof PROJECTS_BY_CATEGORY>('all');

  const allFilters: { id: keyof typeof PROJECTS_BY_CATEGORY; label: string }[] = [
    { id: 'all', label: 'All Projects' },
    { id: 'devops', label: 'DevOps' },
    { id: 'fullstack', label: 'Full-Stack' },
  ];
  const filters = allFilters.filter((f) => PROJECTS_BY_CATEGORY[f.id]?.length > 0);

  const getProjects = () => PROJECTS_BY_CATEGORY[activeFilter] || PROJECTS_BY_CATEGORY.all;
      ...
      {filters.length > 1 && (
        <section className="py-12 bg-bg-surface/30">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
              {filters.map((filter) => {
                const count = PROJECTS_BY_CATEGORY[filter.id]?.length ?? 0;
                return (
                  <button
                    key={filter.id}
